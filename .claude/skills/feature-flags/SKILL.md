...
description: Implements feature flags for A/B testing, progressive rollouts, and conditional feature access using PostHog with both server-side (SSR/API) and client-side (React hooks) evaluation. Use when adding feature flags, implementing A/B tests, setting up progressive rollouts, or debugging flag evaluation.
allowed-tools:
  - Read
  - Edit
  - Write
