...
description: Implements feature flags for A/B testing, progressive rollouts, and conditional feature access using PostHog with both server-side (SSR/API) and client-side (React hooks) evaluation. Use when implementing feature flags, setting up A/B tests, creating progressive rollouts, or debugging flag evaluation.
allowed-tools:
  - Read
  - Edit
  - Write
...
For unit test mocks, client-side test helpers, and integration test patterns, see [testing.md](testing.md).

**Quick summary:**
- Unit tests: mock `@repo/api/lib/feature-flags` with `vi.mock()`
- Client tests: wrap with `<FeatureFlagProvider bootstrappedFlags={...}>`
- Integration: service auto-disables in `NODE_ENV=test` (returns defaults without calling PostHog)
