...
For async file processing workflows (e.g., invoice processor, news analyzer), see [storage-job-pattern.md](storage-job-pattern.md).

**Key principle**: Upload file to storage first, then pass path reference to job queue (not base64 data).

**Quick workflow**:
1. Frontend requests signed upload URL
2. Frontend uploads file directly to storage
3. Frontend creates job with `{ filePath, bucket, mimeType }`
4. Job processor fetches file from storage using path reference
5. Processor returns result

This pattern avoids passing large base64-encoded data through the job queue and enables file reprocessing without re-uploading.
