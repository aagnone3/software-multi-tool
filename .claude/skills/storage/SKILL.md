...
Key differences from direct uploads:
- Generate unique UUID filenames (not fixed names)
- Pass `{ filePath, bucket, mimeType }` to job
- Processor fetches from storage when ready
- Consider cleanup policies for processed files

For complete architecture flow, endpoint patterns, and code examples, see [job-upload-pattern.md](job-upload-pattern.md).
