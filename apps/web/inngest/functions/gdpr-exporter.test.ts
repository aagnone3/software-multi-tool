...
const mockProcessGdprExportJob = vi.hoisted(() => vi.fn());
...
vi.mock("@repo/api/modules/gdpr-exporter", () => ({
	processGdprExportJob: mockProcessGdprExportJob,
}));
...
import { gdprExporter } from "./gdpr-exporter";

describe("gdprExporter inngest function", () => {
	const mockJob = { id: "job-1", toolSlug: "gdpr-exporter" };
	const toolJobId = "job-1";

	beforeEach(() => {
		vi.clearAllMocks();
		mockGetToolJobById.mockResolvedValue(mockJob);
		mockProcessGdprExportJob.mockResolvedValue({
			success: true,
			output: { result: "ok" },
		});
	...
	it("marks job completed with output", async () => {
		...
		mockProcessGdprExportJob.mockResolvedValue({ success: true, output });
		const step = {
			run: vi.fn(async (_: string, fn: () => unknown) => fn()),
		};
		await (gdprExporter as unknown as Function)({
	...
	it("uses 'Unknown error' when error message is missing", async () => {
		mockProcessGdprExportJob.mockResolvedValue({ success: false });
		const step = {
			run: vi.fn(async (_: string, fn: () => unknown) => fn()),
		};
		await (gdprExporter as unknown as Function)({
