...
const mockProcessMeetingJob = vi.hoisted(() => vi.fn());
...
vi.mock("@repo/api/modules/meeting-summarizer", () => ({
	processMeetingJob: mockProcessMeetingJob,
}));
...
describe("meetingSummarizer inngest function", () => {
	const mockJob = { id: "job-1", toolSlug: "meeting-summarizer" };
	const toolJobId = "job-1";

	beforeEach(() => {
		vi.clearAllMocks();
		mockGetToolJobById.mockResolvedValue(mockJob);
		mockProcessMeetingJob.mockResolvedValue({
			success: true,
			output: { result: "ok" },
		});
	...
	it("marks job completed with output", async () => {
		...
		mockProcessMeetingJob.mockResolvedValue({ success: true, output });
		const step = {
			run: vi.fn(async (_: string, fn: () => unknown) => fn()),
		};
		await (meetingSummarizer as unknown as Function)({
	...
	it("uses 'Unknown error' when error message is missing", async () => {
		mockProcessMeetingJob.mockResolvedValue({ success: false });
		const step = {
			run: vi.fn(async (_: string, fn: () => unknown) => fn()),
		};
		await (meetingSummarizer as unknown as Function)({
