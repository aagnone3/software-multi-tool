...
const mockProcessFeedbackJob = vi.hoisted(() => vi.fn());
...
vi.mock("@repo/api/modules/feedback-analyzer", () => ({
	processFeedbackJob: mockProcessFeedbackJob,
}));
...
describe("feedbackAnalyzer inngest function", () => {
	const mockJob = { id: "job-1", toolSlug: "feedback-analyzer" };
	const toolJobId = "job-1";

	beforeEach(() => {
		vi.clearAllMocks();
		mockGetToolJobById.mockResolvedValue(mockJob);
		mockProcessFeedbackJob.mockResolvedValue({
			success: true,
			output: { result: "ok" },
		});
	...
	it("runs all three steps", async () => {
		...
		await (feedbackAnalyzer as unknown as Function)({
	...
	it("returns success when job completes", async () => {
		...
		const result = await (feedbackAnalyzer as unknown as Function)({
			event: { data: { toolJobId } },
			step,
		});
	...
	it("throws if job not found during validate step", async () => {
		...
		await expect(
			(feedbackAnalyzer as unknown as Function)({
				event: { data: { toolJobId } },
				step,
			}),
	...
	it("marks job completed with output", async () => {
		...
		mockProcessFeedbackJob.mockResolvedValue({ success: true, output });
		const step = {
			run: vi.fn(async (_: string, fn: () => unknown) => fn()),
		};
		await (feedbackAnalyzer as unknown as Function)({
	...
	it("uses 'Unknown error' when error message is missing", async () => {
		mockProcessFeedbackJob.mockResolvedValue({ success: false });
		const step = {
			run: vi.fn(async (_: string, fn: () => unknown) => fn()),
		};
		await (feedbackAnalyzer as unknown as Function)({
