...
const mockProcessExpenseJob = vi.hoisted(() => vi.fn());
...
vi.mock("@repo/api/modules/expense-categorizer", () => ({
	processExpenseJob: mockProcessExpenseJob,
}));
...
describe("expenseCategorizer inngest function", () => {
	const mockJob = { id: "job-1", toolSlug: "expense-categorizer" };
	const toolJobId = "job-1";

	beforeEach(() => {
		vi.clearAllMocks();
		mockGetToolJobById.mockResolvedValue(mockJob);
		mockProcessExpenseJob.mockResolvedValue({
			success: true,
			output: { result: "ok" },
		});
	...
	it("runs all three steps", async () => {
		...
		await (expenseCategorizer as unknown as Function)({
	...
	it("returns success when job completes", async () => {
		...
		const result = await (expenseCategorizer as unknown as Function)({
			event: { data: { toolJobId } },
			step,
		});
	...
	it("throws if job not found during validate step", async () => {
		...
		await expect(
			(expenseCategorizer as unknown as Function)({
				event: { data: { toolJobId } },
				step,
			}),
	...
	it("marks job completed with output", async () => {
		...
		mockProcessExpenseJob.mockResolvedValue({ success: true, output });
		const step = {
			run: vi.fn(async (_: string, fn: () => unknown) => fn()),
		};
		await (expenseCategorizer as unknown as Function)({
	...
	it("uses 'Unknown error' when error message is missing", async () => {
		mockProcessExpenseJob.mockResolvedValue({ success: false });
		const step = {
			run: vi.fn(async (_: string, fn: () => unknown) => fn()),
		};
		await (expenseCategorizer as unknown as Function)({
