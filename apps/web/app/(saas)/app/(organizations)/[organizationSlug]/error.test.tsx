...
describe("OrganizationError", () => {
	it("renders error heading", () => {
		render(
			<OrganizationError error={new Error("test")} reset={() => {}} />,
		);
		expect(
			screen.getByText("Organization page failed to load"),
		).toBeTruthy();
	});

	it("calls reset when Try again is clicked", async () => {
		const reset = vi.fn();
		render(<OrganizationError error={new Error("test")} reset={reset} />);
