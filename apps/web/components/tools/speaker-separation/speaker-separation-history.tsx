...
export function SpeakerSeparationHistory() {
	...
	const sessionId = useMemo(() => {
		...
		return (
			localStorage.getItem("speaker-separation-session-id") ?? undefined
		);
	}, []);
	...
	const filteredJobs = useMemo(() => {
		let jobs = (data?.jobs ?? []) as SpeakerSeparationJob[];
	...
	const columns: ColumnDef<SpeakerSeparationJob>[] = useMemo(
		() => [
			{
				...
				cell: ({ row }) => {
					...
					return (
						<div className="flex items-center gap-2 max-w-[200px]">
							<AudioLines className="size-4 text-muted-foreground flex-shrink-0" />
			...
			},
			{
				...
				header: "Speakers",
				cell: ({ row }) => {
					const count = getSpeakerCount(row.original);
					if (count === null) {
						return (
							<span className="text-sm text-muted-foreground">
								-
